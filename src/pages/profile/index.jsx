...
import { ADD_PROFILE_IMAGE_ROUTE, REMOVE_PROFILE_IMAGE, UPDATE_PROFILE_ROUTE } from '@/utils/contants';
        ...
        <div className='grid grid-cols-2'>
          ...
          >
            <Avatar className="h-32 w-32 md:w-48 md:h-48 rounded-full overflow-hidden">
              {image ? (
                ...
                <div
                  className={`uppercase h-32 w-32 md:w-48 md:h-48 text-5xl border-[3px] flex items-center justify-center ${getColor(
                    selectedColor
                  )}`}
                >
                  {firstName || userInfo?.email?.split("")?.shift() || ""}
                </div>
              )}
            </Avatar>
            {hovered && (
          ...
          <div className='flex min-w-32 md:min-w-64 flex-col gap-5 text-white items-center justify-center'>
            ...
            <div className='w-full flex gap-5'>
                 {
                  colors.map((color,index)=>(<div className={`${color} h-8 w-8 rounded-full cursor-pointer transition-all duration-300 ${selectedColor===index ? "outline outline-white/50":""}`} key={index} onClick={() => setSelectedColor(index)}>
