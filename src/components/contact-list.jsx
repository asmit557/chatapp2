    ...
    <div className='mt-5'>
      {
          contacts.map((contact)=>(
              <div key={contact._id} className={`pl-10 py-2 
              ...
              onClick={()=>handleClick(contact)}>
                   <div className='flex gap-5 items-center justify-start text-neutral-300'>
                          ...
                          !isChannel && (
                              <Avatar className="h-10 w-10 rounded-full overflow-hidden">
                                                       {contact.image ? (
                                                         ...
                                                         <div
                                                         className={`${selectedChatData && selectedChatData._id === contact._id ? "bg-[ffffff22] border-2 border-white/70":getColor(contact.color)} uppercase h-10 w-10 text-lg border-[1px] flex items-center justify-center rounded-full`}
                                                                >
                        ...
                        {isChannel ? (<span>{contact.name}</span>) : (
                           <span>{contact.firstName || contact?.email?.split("")?.shift() || ""}</span>
                        )}
                   </div>
              </div>
          ))
